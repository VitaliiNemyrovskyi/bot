...
/**
 * GET /api/bybit/asset-info
 *
 * Retrieves asset information from Bybit using stored API keys.
 * This endpoint provides detailed information about assets in different account types,
 * including available balance, locked amount, and other asset-related data.
 *
 * Query Parameters:
 * - accountType (optional): SPOT, CONTRACT, UNIFIED, INVESTMENT, OPTION, FUND
 *   If not specified, returns asset info for all account types
 * - coin (optional): Specific coin to query (e.g., BTC, ETH, USDT)
 *   If not specified, returns information for all coins
 *
 * Authentication: Required (Bearer token)
 *
 * This endpoint loads the user's saved Bybit API credentials from the database
 * and uses them to fetch asset information.
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    const accountType = searchParams.get('accountType')?.toUpperCase() || undefined;
    ...
    // 5. Fetch asset information from Bybit
    const assetInfo = await bybitService.getAssetInfo(accountType, coin);
