/**
 * Bybit Service with State Caching Support
 *
 * Extension of BybitService that supports loading time sync state from cache
 * to eliminate 1-2 second initialization delay in serverless environments.
 */

import { BybitService, BybitConfig } from './bybit';
import { ConnectorStateCacheService, ConnectorState } from '@/services/connector-state-cache.service';

export interface BybitConfigWithCache extends BybitConfig {
  userId?: string;
  credentialId?: string;
  skipTimeSync?: boolean; // If true, try to load from cache
}

export class BybitServiceWithCache extends BybitService {
  private userId?: string;
  private credentialId?: string;

  constructor(config: BybitConfigWithCache) {
    super(config);
  ...
  async initializeWithCache(): Promise<boolean> {
    ...
    if (this.userId && this.credentialId) {
      ...
      if (cached) {
        ...
        console.log(`[BybitServiceWithCache] Loaded cached time sync (age: ${age}s, offset: ${cached.timeOffset}ms)`);
    ...
    console.log(`[BybitServiceWithCache] Time sync completed in ${syncDuration}ms`);
  ...
  async cacheTimeOffset(): Promise<void> {
    ...
    try {
      await ConnectorStateCacheService.set(
        ...
        this.credentialId,
        'BYBIT',
        (this as any).config.testnet ? 'TESTNET' : 'MAINNET',
        state
      );

      console.log(`[BybitServiceWithCache] Cached time offset: ${state.timeOffset}ms`);
    } catch (error: any) {
      console.error(`[BybitServiceWithCache] Failed to cache time offset:`, error.message);
    }
  }
