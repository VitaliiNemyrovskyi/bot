...
function generateSignature(params: Record<string, any>, apiSecret: string): string {
  ...
  console.log('Signature query:', queryString);
...
async function testCorrectParams() {
  try {
    ...
    // Try with BUY instead of BID
    console.log('=== Test: Using side=BUY ===\n');
    const timestamp = Date.now();
    const orderParams = {
      symbol: 'BTC-USDT',
      side: 'BUY',  // Changed from BID
      positionSide: 'LONG',
      type: 'MARKET',
      quantity: 0.001,  // Number instead of string
      timestamp
    };
