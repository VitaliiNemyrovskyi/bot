...
async function main() {
  console.log('🔍 HIGH FUNDING RATE TIMING OPTIMIZATION (|Funding Rate| >= 1%)');
  console.log('Testing only recordings with absolute funding rate >= 1%\n');

  // Get recordings with high funding rates
  ...
  console.log(`Found ${recordings.length} recordings with |funding rate| >= 1%\n`);

  if (recordings.length === 0) {
    console.log('❌ No recordings found with high funding rates');
    return;
  }

  // Show which recordings we're using
  console.log('📋 High funding rate recordings:');
  recordings.forEach((r, i) => {
    console.log(`${i + 1}. ${r.symbol} @ ${r.exchange} - Funding: ${(r.fundingRate * 100).toFixed(4)}%`);
  });
  console.log('');

  // Test entry times: 10ms, 20ms, 50ms, 100ms, 150ms, 200ms, 250ms, 300ms, 400ms, 500ms, 750ms, 1000ms, 1500ms, 2000ms
  const entryTimes = [10, 20, 50, 100, 150, 200, 250, 300, 400, 500, 750, 1000, 1500, 2000];

  // Test exit times: 1s, 2s, 3s, 5s, 7s, 10s, 15s, 20s, 30s
  const exitTimes = [1000, 2000, 3000, 5000, 7000, 10000, 15000, 20000, 30000];
  ...
  console.log('🏆 TOP 30 BEST TIMING COMBINATIONS (by Average Profit) - HIGH FUNDING RATES');
  console.log('═'.repeat(120));
